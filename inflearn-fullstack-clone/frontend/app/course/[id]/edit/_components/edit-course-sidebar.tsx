...
function getStepStatus(pathname: string, id: string, key: string, index: number) {
...
export default function EditCourseSidebar() {
  ...
  return (
    <Card className="min-w-60 w-60 p-6 flex flex-col gap-8 bg-white border-none rounded-lg shadow-none">
      {steps.map((section) => (
                ...
                <li key={item.key} className="flex items-start gap-3">
                  <Link
                    ...
                    className={`flex items-center gap-3 py-2 px-3 rounded-lg w-full transition-colors
                      ${isActive ? 'bg-green-50 text-black font-bold border border-green-500' : ''}
                  ...
                  >
                    <span className="flex items-center justify-center w-7 h-7">
                      <span
                        className={`w-6 h-6 flex items-center justify-center rounded-full border
                            ${isActive ? 'border-green-500 text-green-600' : 'border-gray-300 text-gray-400'}
                          `}
                      >
                        {stepNumber}
                      </span>
                    ...
                    <div className="flex flex-col">
                      ...
                      {item.description && <span className="text-xs text-gray-400 mt-1">{item.description}</span>}
                    </div>
                  </Link>
                </li>
              );
