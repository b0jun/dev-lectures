        ...
        <TableBody>
          {courses && courses.length > 0 ? (
            courses.map((course: Course) => {
              ...
              const status = course.status === 'PUBLISHED' ? '게시중' : '임시저장';
              return (
                <TableRow key={course.id}>
                  ...
                  <TableCell>
                    {discountPrice ? (
                      <>
                        <span className="line-through text-gray-400 mr-1">₩{price.toLocaleString()}</span>
                        <span className="text-green-700 font-bold">₩{discountPrice.toLocaleString()}</span>
                      </>
                    ) : price ? (
                  ...
                  <TableCell className="flex flex-col gap-2 justify-center h-full">
                    ...
                    <Button
                      onClick={() => router.push(`/course/${course.id}/edit/course_info`)}
                      variant="outline"
                      size="sm"
                    >
